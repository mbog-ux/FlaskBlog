...
from flaskblog.forms import RegistrationForm,LoginForm,UpdateAccountForm,PostForm,SendResetForm, PasswordResetForm
...
def send_email(user):
    ...
    {url_for('reset_password',token=user.get_reset_token(),_external=True)}
    '''
    mail.send(msg)


@app.route('/password/send_reset_token',methods=['GET','POST'])
...
@app.route('/password/reset_password/<token>',methods=['GET','POST'])
